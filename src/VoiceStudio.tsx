// Estudio de voz — editor con MARCADORES INLINE (como Flow, pero para voz).
// Marcás énfasis, pausas y tono (etiquetas v3) dentro del texto. Genera contra
// el media-service (la key vive allá). Una generación → mp3 reproducible/descargable.
import { useRef, useState } from 'react';
import { Mic, Download, Play, Pause } from 'lucide-react';
import { NARRATORS, REGION_LABEL, type Region } from './data/narrators';
import { NARRATION, TTS_SERVICE_URL } from './data/narrationText';
...
export default function VoiceStudio() {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 360px', gap: 28, alignItems: 'start', fontFamily: FONT_SANS, color: '#fff' }}>
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap', marginBottom: 10 }}>
          ...
          {MARKERS.map((m) => (<button key={m.label} onClick={() => insertAtCursor(m.ins)} style={btn}>{m.label}</button>))}
        </div>
        ...
        <div style={{ fontSize: 11.5, color: 'rgba(255,255,255,0.45)', marginTop: 8, lineHeight: 1.6 }}>
          <b>Énfasis</b>: seleccioná texto y tocá ÉNFASIS (MAYÚSCULAS). · <b>Pausa/Tono</b>: cursor donde querés y tocá el marcador. Solo <b>v3</b> respeta los tonos.
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)' }}>Cargar:</span>
          {Object.keys(NARRATION).map((k) => (<button key={k} onClick={() => setText(NARRATION[k].join('\n'))} style={{ ...btn, fontSize: 11, padding: '4px 10px', borderRadius: 999 }}>{k}</button>))}
        </div>
      </div>
      ...
      <div style={{ position: 'sticky', top: 18, display: 'flex', flexDirection: 'column', gap: 16, borderRadius: 16, padding: 18, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {[{ id: 'eleven_v3', label: 'v3 (marcadores)' }, { id: 'eleven_multilingual_v2', label: 'v2 (estable)' }].map((m) => (
        ...
        <div>
          ...
          <div style={{ fontSize: 10.5, color: 'rgba(255,255,255,0.4)', marginTop: 2 }}>bajo = más expresivo</div>
        </div>
        <button onClick={generate} disabled={busy} style={{ cursor: busy ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9, padding: 14, borderRadius: 12, border: 'none', background: busy ? 'rgba(255,255,255,0.15)' : BRAND.gold, color: busy ? '#fff' : BRAND.ink, fontWeight: 800, fontSize: 15.5 }}>
        ...
        {url && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <a href={url} download="voz.mp3" style={{ flex: 1, textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: 11, borderRadius: 11, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', color: '#fff', fontWeight: 700, fontSize: 13.5 }}><Download size={16} /> Descargar mp3</a>
          </div>
        )}
