...
import { pasosVisibles, pasoHabilitado, type PasoId, type EstadoPaso, type TipoComercial, type Comercial } from './lib/comercial';
...
export default function PipelineStepper({ tipo, comercial, activePaso, onPick }: Props) {
  const pasos = pasosVisibles(tipo);
  const estadoDe = (p: PasoId): EstadoPaso => comercial?.estados?.[p] ?? 'pendiente';
  return (
    <nav className="pw-steps" aria-label="Pasos del comercial">
      {pasos.map((p, i) => {
        ...
        // GATE del spine (HANDOFF §6: "spine izq = pasosVisibles(tipo) + gate pasoHabilitado"): un paso
        // se abre cuando el ANTERIOR VISIBLE ya está ≥ 'generado'. Sin eso el spine dejaba entrar a
        // Storyboard sin guion y los pasos disparaban la IA sin insumos. Sin comercial todavía, sólo
        // el primero (negocio) es navegable.
        const habilitado = comercial ? pasoHabilitado(comercial, p) : i === 0;
        return (
          <button
            key={p}
            ...
            onClick={() => onPick(p)}
            disabled={!habilitado}
            title={habilitado ? M.label : `Completá antes ${META[pasos[i - 1]].label}`}
